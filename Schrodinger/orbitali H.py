...
plt.errorbar(xp, -psi[:,m-1]+avals[m-1], fmt='.',  label='$\psi(x)$ numerica %ds' %m)
...
plt.plot(x , x*G(x, m, 0)+avals[m-1], color='red', label='$\psi(x)$ analitica %ds' %m)
plt.grid()
plt.legend(loc='best')
...
plt.errorbar(xp, -psi[:,m-1]-(x*G(x, m, 0)), fmt='.')
plt.grid()
